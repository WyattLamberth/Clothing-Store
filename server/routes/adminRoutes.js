...
router.get('/reports/inventory', async (req, res) => {
  ...
  try {
    ...
    const [totalOrdered] = await connection.execute(`
      SELECT SUM(quantity) as total_ordered
      FROM order_items
    `);

    // Calculate total items returned
    const [totalReturned] = await connection.execute(`
      SELECT SUM(quantity) as total_returned
      FROM return_items
    `);

    // Calculate overall return rate
    const totalOrderedCount = totalOrdered[0].total_ordered || 0;
    const totalReturnedCount = totalReturned[0].total_returned || 0;
    const overallReturnRate = totalOrderedCount > 0
    ...
    const summary = {
      ...
      overallReturnRate: parseFloat(overallReturnRate) // Return rate as a percentage
    };
