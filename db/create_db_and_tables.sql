...
DELIMITER //
CREATE TRIGGER check_inventory AFTER UPDATE ON PRODUCTS
FOR EACH ROW
BEGIN
    IF NEW.Stock_Quantity < NEW.Reorder_Threshold THEN
        INSERT INTO REORDER_ALERTS (Product_ID, Alert_Date, Quantity_To_Reorder)
        VALUES (NEW.Product_ID, CURDATE(), NEW.Reorder_Threshold - NEW.Stock_Quantity);
    END IF;
END//
DELIMITER ;

-- Add trigger for return policy (7-day limit)
DELIMITER //
CREATE TRIGGER check_return_date BEFORE INSERT ON RETURNS
FOR EACH ROW
BEGIN
    DECLARE order_date DATE;
    SELECT Order_Date INTO order_date FROM ORDERS WHERE Order_ID = NEW.Order_ID;
    IF DATEDIFF(NEW.Return_Date, order_date) > 7 THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Returns are only allowed within 7 days of the original purchase date';
    END IF;
END//
DELIMITER ;

-- Add trigger for customer loyalty program
DELIMITER //
CREATE TRIGGER update_customer_status AFTER INSERT ON ORDERS
FOR EACH ROW
BEGIN
    DECLARE order_count INT;
    SELECT COUNT(*) INTO order_count FROM ORDERS WHERE Customer_ID = NEW.Customer_ID;
    IF order_count = 10 THEN
        UPDATE CUSTOMER SET Status = 'Premium' WHERE Customer_ID = NEW.Customer_ID;
    END IF;
END//
DELIMITER ;